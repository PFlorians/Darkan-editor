...
import darkan.editor.plugin.model.PluginWrapper
...
object PluginManager {
    ...
    private fun findPlugins() : Set<Path> {
        ...
        val pluginDir = Paths.get("plugins-runtime")
        ...
        Files.walk(pluginDir).filter { it.fileName.toString().endsWith(".jar") }.distinct().forEach { set.add(it) }
        return set
    }
